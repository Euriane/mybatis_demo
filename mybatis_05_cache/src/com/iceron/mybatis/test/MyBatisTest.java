...
public class MyBatisTest {
	...
	public void test28() throws IOException {
		...
		try {
			...
			//4、会为接口自动的创建一个代理对象，代理对象去执行增删改查方法
			Employee emp = mapper.getEmpById(25);    
			System.out.println(emp);
			
			Employee emp2 = mapper.getEmpById(25);    //执行该语句时，并没有去数据库查询. 而是直接从一级缓存中拿到了之前查询到的数据。
	...
	public void test29() throws IOException {
		...
		//SqlSession openSession = sqlSessionFactory.openSession();  //获取到的openSession 不会自动提交
		SqlSession openSession = sqlSessionFactory.openSession(true);  //设置了自动提交，无需再手动提交
		SqlSession openSession02 = sqlSessionFactory.openSession(true);
		try {
			// 3、获取接口的实现类对象
			EmployeeMapper mapper = openSession.getMapper(EmployeeMapper.class);
			...
			Employee emp = mapper.getEmpById(25);    
			System.out.println(emp);
			openSession.close();   // 只有一级缓存失效了，二级缓存中才会有这个数据。
			
			//在sql 映射文件开启二级缓存后 <cache eviction="FIFO" flushInterval="60000" readOnly="false" size="1024"></cache>， 
			//下面的查询不会去数据库查询(没有发送新的sql语句)，而是直接从二级缓存中获取
			Employee emp02 = mapper02.getEmpById(25);     
